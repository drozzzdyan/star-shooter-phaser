...
export default class StatisticsScene extends Phaser.Scene {
  ...
  renderStatistics() {
    ...
    this.btnMenu = new Btn(this, midX, midY - 100, 'btn', 'btn_menu', 'btn_menu_active', 'Menu');
    ...
    const leftOffset = midX - 0;

    if (records.length > 0) {
      for (let i = 0; i < records.length; i++) {
        const el = records[i];
        
        this.add.sprite(leftOffset - 50, midY + i * marginBottomText + 15, 'player', `player${records[i].shipType}`).setScale(0.35);
        this.add.text(leftOffset, midY + i * marginBottomText, `${records[i].record}`, {
          fontFamily: 'Kanit',
          fontSize: '30px',
          color: '#b755b9',
        });
